  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrollProgress = Math.max(
        ...
        Math.min(
          1,
          (viewportTrigger - containerRect.top) / (lastDotPosition + 100),
        ),
      );
      ...
      <div className="space-y-16 md:space-y-20">
        {experience.map((exp, index) => {
          ...
          return (
            ...
            >
              <p
                className={`text-xs md:text-right leading-5 transition-colors duration-300 ${isActive ? "text-muted" : "text-muted/50"}`}
              >
                {exp.period}
              </p>
              ...
              <div>
                <h3
                  className={`text-sm font-medium mb-1 leading-5 transition-colors duration-300 ${isActive ? "text-foreground" : "text-foreground/70"}`}
                >
                  {exp.company}
                </h3>
                <p className="text-xs text-muted mb-3">{exp.role}</p>
                <p
                  className={`text-sm leading-relaxed transition-colors duration-300 ${isActive ? "text-muted" : "text-muted/50"}`}
                >
                  {exp.description}
                </p>
              </div>
                ...
                {project.links && project.links.length > 0 && (
                  <div className="flex gap-6 shrink-0">
                    {project.links.map((link) => {
                      ...
                      const linkClass =
                        "text-xs text-foreground hover:text-accent transition-colors duration-300 flex items-center gap-1.5 group/link";
                      const content = (
                        <>
                          {link.label}
                          <svg
      ...
      <main className="max-w-4xl mx-auto px-6">
        ...
        <section id="work" className="pb-24 md:pb-32 scroll-mt-24">
          ...
          >
            ...
            <span className="text-xs text-muted/50">Click to expand</span>
          </div>
